...
const Nav = (props: { items: NavpointProps[] }): ReactElement => {
  ...
  useEffect(() => {
    ...
    const backdropBehaviour = (): void => {
      setActive(!active);
    };

    if (active) {
      backdrop.current?.classList.remove("d-none");
      navmenu.current?.classList.remove("d-none");
      backdrop.current?.addEventListener("click", backdropBehaviour);
    } else {
      backdrop.current?.classList.add("d-none");
      navmenu.current?.classList.add("d-none");
      backdrop.current?.removeEventListener("click", backdropBehaviour);
    }
  }, [active]);
